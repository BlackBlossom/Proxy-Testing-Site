...
export default function TermsEditorPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editIndex !== null) {
        ...
        await axios.put(`/content/terms/${formData.id}`, formData);
        const updatedSections = [...sections];
        updatedSections[editIndex] = formData;
        setSections(updatedSections);
      } else {
        // Add new section
        const { data } = await axios.post('/content/terms', formData);
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`/content/terms/${sections[sectionToDelete].id}`);
    ...
    <div className="min-h-screen flex flex-col bg-[#F7F5FF] p-8 px-16">
      ...
      <AnimatePresence>
        {error && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className="p-4 mb-6 self-center w-200 flex items-center gap-3 bg-[#F7F5FF] border-l-4 border-[#412D99] text-[#1B1340] rounded-lg"
          >
      ...
      <div className="flex flex-col gap-6 justify-center items-center w-200 mx-auto mb-8">
        {sections.length === 0 ? (
          ...
          >
            <p className="text-lg font-medium">No terms & conditions sections found.</p>
